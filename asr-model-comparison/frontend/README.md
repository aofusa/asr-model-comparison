...
ビルド成果物は `dist/` に出力されます。`dist/` は生成物のため Git 管理対象には含めません。
...
これにより、ビルド成果物が自動的に `backend/static/` にコピーされ、シングルアプリモードで利用可能になります。`backend/static/` も再生成されるため Git 管理対象には含めません。
