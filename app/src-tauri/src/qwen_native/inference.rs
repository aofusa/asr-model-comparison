...
use crate::qwen_native::config::AsrConfig;
use crate::qwen_native::decoder::{
    compute_mrope_cos_sin, create_causal_mask, KvCache, TextDecoder,
};
use crate::qwen_native::encoder::AudioEncoder;
use crate::qwen_native::error::AsrError;
use crate::qwen_native::mel::{load_audio_wav, MelExtractor};
...
///
/// ```
/// # use amcp_tauri::qwen_native::TranscribeOptions;
...
pub struct SpeechTranslationResult {
    pub transcript: TranscribeResult,
    pub translated: Option<TranscribeResult>,
}
...
impl AsrInference {
    pub fn load(model_dir: &Path, device: Device) -> crate::qwen_native::Result<Self> {
        info!("Loading config...");
    ...
    /// # use std::path::Path;
    /// let engine = amcp_tauri::qwen_native::AsrInference::from_pretrained(
    ///     "Qwen/Qwen3-ASR-0.6B",
    ///     Path::new("models/"),
    ///     Device::Cpu,
    /// )?;
    /// # Ok::<(), amcp_tauri::qwen_native::AsrError>(())
    /// ```
    ...
    ) -> crate::qwen_native::Result<TranscribeResult> {
    ...
    pub fn transcribe_samples_with_translation(
        &self,
        samples: &[f32],
        language: Option<&str>,
        target_language: Option<&str>,
        max_new_tokens: usize,
    ) -> crate::qwen_native::Result<SpeechTranslationResult> {
        let inner = self
            .inner
            .lock()
            .map_err(|_| AsrError::Inference(anyhow::anyhow!("mutex poisoned")))?;
        inner
            .run_speech_translation(samples, language, target_language, max_new_tokens)
            .map_err(AsrError::Inference)
    }
}
...
impl AsrInferenceInner {
    ...
    pub(crate) fn run_speech_translation(
        &self,
        samples: &[f32],
        language: Option<&str>,
        target_language: Option<&str>,
        max_new_tokens: usize,
    ) -> anyhow::Result<SpeechTranslationResult> {
        let audio_embeds = self.encode_audio(samples)?;
        let transcript_ids = self.generate(&audio_embeds, language, None, max_new_tokens)?;
        let transcript = self.decode_result(&transcript_ids, language)?;
        let translated = if target_language.is_some() && target_language != language {
            let translated_ids =
                self.generate(&audio_embeds, target_language, None, max_new_tokens)?;
            Some(self.decode_result(&translated_ids, target_language)?)
        } else {
            None
        };

        Ok(SpeechTranslationResult {
            transcript,
            translated,
        })
    }
