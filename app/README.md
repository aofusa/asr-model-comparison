...
- Whisper、Qwen3-ASR Candle、Voxtral Realtime ExecuTorch/llama.cppはfeature有効時に実推論へ接続します。翻訳はQwen3-ASR Candleの出力言語プロンプト、またはVoxtral patched GGUFのtext decoderによるmodel-native翻訳を優先します。macOSのVoxtral Metal実行はExecuTorch runner経路を優先します。
...
- Qwen3-ASR Candleの出力言語プロンプト、Voxtral Realtime ExecuTorch MetalによるASR、およびVoxtral Realtime patched llama.cppによるASR/翻訳
- 単一モデルロード制約の管理
...
- `/api/status`とWS応答でのランタイムバックエンド状態 (`whisper-rs` / `qwen-native` / `voxtral-executorch` / `voxtral-llamacpp` / `placeholder`) と、モデルファイル/ディレクトリ/model-native翻訳のartifact診断
- Qwen3-ASR Candle / Voxtral ExecuTorch / Voxtral patched llama.cpp のfeature境界と設定検証 (`AMCP_QWEN_*`、`AMCP_VOXTRAL_*`)
- Qwen3-ASR Candleのモデルロード、自動キャッシュ、モデルディレクトリ検証、実音声サンプル推論呼び出し
- Voxtral Realtime ExecuTorch Metal runner構成、GGUF/mmproj構成の設定解決、dual-stream音声デコード、同一GGUF text decoderによる翻訳
- macOS Apple Silicon向けの `full-runtime-macos` feature、Whisper Metal、Qwen Candle Metal、Voxtral ExecuTorch Metal診断
...
- Rust 1.95+ / Cargo
- Node.js 24+ / npm
- CMake 3.25+（Whisper Metal / patched llama.cppなどのネイティブビルドに必要）
- Playwrightで利用するブラウザ
...
macOS Apple Siliconでは `qwen-metal` featureを使います。Metal deviceを初期化できない環境ではCPUへフォールバックします。

```bash
npm run server:qwen:metal
```
...
### Voxtral Realtime ExecuTorch Metal実推論（macOS）

macOS Apple Siliconでは、Voxtral RealtimeのMetal backendとしてExecuTorch runnerを使えます。Windows向けのpatched `llama.cpp` と異なり、runnerは外部プロセスとして起動し、Rust側で前処理済みの16kHz mono音声を一時WAVとして渡します。`target_language` 指定時のmodel-native翻訳はpatched llama.cpp経路のみ対応です。
...
Windowsでは、Voxtral Realtime対応パッチ済み `llama.cpp` / `libmtmd` をアプリ本体へC ABIブリッジでリンクする経路を標準で使います。macOSでも代替経路として利用でき、`target_language` 指定時のmodel-native翻訳はこのruntimeへルーティングします。
...
macOSでMetal offloadを使う場合は、パッチ済み `llama.cpp` をMetal有効の共有ライブラリとしてビルドします。`build:macos:app` は `libllama.dylib`、`libmtmd.dylib`、`libggml*.dylib`、`default.metallib` が見つかれば `app/dist/` と `.app` の実行ファイル側へコピーします。
...
### macOS配布用のTauriアプリを作成

macOS / Apple Silicon向けには `full-runtime-macos` featureを使います。WhisperはMetal、Qwen3-ASRはCandle Metal、VoxtralはExecuTorch Metal runnerを優先します。Voxtralのrunnerやモデルが未準備の場合でも、`/api/status` で未設定artifactとして診断できます。Whisper/Qwenのみのfeature overrideでビルドすることもできます。
`build:macos:app` は `MACOSX_DEPLOYMENT_TARGET=11.0` を既定で設定します。これは `whisper.cpp` / ggml のC++17 filesystem利用に必要です。

```bash
cd app
npm run build:macos:app

# Voxtral runtimeを含めず、Whisper/Qwen Metalだけを確認する場合
...
```bash
./dist/AMCP-macos --server --port 8000
...
npm run test:voxtral:executorch:compile
...
macOSでは `ja_01.mp3` を使い、Metalが選択されたかを `accelerator` と `chunks[].device`、`runtime_backend` で確認します。モデルが未取得の場合はHugging Face Hubの共通キャッシュへダウンロードされます。

```bash
cd app
npm run validate:macos -- --diagnostics-only --json
...
Qwen3-ASRは `qwen-metal` featureでCandle Metalを試し、Metal初期化に失敗した場合はCPUへフォールバックします。Voxtralは `AMCP_VOXTRAL_EXECUTORCH_*` 一式が存在する場合にExecuTorch Metal runtimeとして診断されます。patched llama.cpp経路を使う場合は `AMCP_VOXTRAL_PATCHED_LLAMA_LINK_METAL=1` または `libggml-metal.dylib` の検出によりMetal providerを有効化します。
...
- `voxtral`: macOS ExecuTorch Metal runner、またはpatched Voxtral Realtime `llama.cpp` / `libmtmd`
...
- Qwen3-ASR
  ...
  - macOS: Metal -> CPU (`qwen-metal` feature時)
