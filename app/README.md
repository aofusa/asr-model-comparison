...
- macOS Apple Silicon向けの `full-runtime-macos` feature、Whisper Metal、Qwen Candle Metal、Voxtral patched llama.cpp Metal診断
...
macOS / Apple Silicon向けには `full-runtime-macos` featureを使います。WhisperはMetal、Qwen3-ASRはCandle Metal、Voxtralはpatched llama.cpp Metalを優先します。Voxtralのpatched runtimeが未準備の場合は、Whisper/Qwenのみのfeature overrideでビルドできます。
...
# Voxtral runtimeが未準備で、Whisper/Qwen Metalだけを確認する場合
...
Qwen3-ASRは `qwen-metal` featureでCandle Metalを試し、Metal初期化に失敗した場合はCPUへフォールバックします。Voxtralは `AMCP_VOXTRAL_PATCHED_LLAMA_LINK_METAL=1` または `libggml-metal.dylib` の検出によりMetal providerを有効化します。
