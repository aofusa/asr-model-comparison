...
| 7 | 機能 | 進行中 | Rust/Tauriプロジェクトを作成し、Pythonバックエンドと既存フロントエンドを移植する。serverモード、host/port指定、CPU/GPU選択と安全なフォールバックを実装する。 | `app/`にRust/Tauri骨格、serverモード、API/WS互換ルート、アクセラレータ選択、単体テストを追加。仕様は `docs/backlog/features/backlog-7-rust-tauri-rewrite.md` に記載。 |
